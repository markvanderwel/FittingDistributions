...
meanBa<-aggregate(totdata[,c("ModelledBa","GrowthBestLog","MortBestLog","IngBestLog",
                             "GrowthBest","MortBest","IngBest")],
                             list(totdata$Lon_Lat,totdata$Pft),mean,na.rm=T)
names(meanBa)<-c("Lon_Lat","Pft","meanModelledBa","meanGrowthBestLog","meanMortBestLog",
                 "meanIngBestLog","meanGrowthBest","meanMortBest","meanIngBest")
...
for (pft in unique(totdata2$Pft)){
  ...
  fb.pars.r<-list(
    ...
    slope_cellmean=c(-100,100,2,0,0,1),
    slope_cellsd=c(1e-6,100,2,1,0,1),
...
######################################################################################################
######################################################################################################
#MODELS OF LOG-TRANSFORMED DEMOGRAPHIC RATES AGAINST MODELLED BASAL AREA
